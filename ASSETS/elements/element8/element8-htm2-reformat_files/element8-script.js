        ...
        function analyze(){
          $( "td" ).each(function( index ) {
            ...
            if ($( this ).text().slice(0,2) == 'A.'){
              answer_one.push($(this).next().text());
              //console.log($(this).next().text());
            }
            if ($( this ).text().slice(0,2) == 'B.'){
              answer_two.push($(this).next().text());
              //console.log($(this).text());
            }
            if ($( this ).text().slice(0,2) == 'C.'){
              answer_three.push($(this).next().text());
              //console.log($(this).text());
            }
            if ($( this ).text().slice(0,2) == 'D.'){
              answer_four.push($(this).next().text());
              //console.log($(this).text());
            }
            if ($( this ).text().indexOf("Answer Key:") >= 0){
